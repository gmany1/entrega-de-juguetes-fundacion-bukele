    ...
    return (
        <div className="space-y-6 animate-fade-in pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white p-6 rounded-xl border border-yellow-200 shadow-sm md:col-span-2">
                    <div className="flex items-center gap-4 mb-4">
                        <div className="bg-yellow-50 w-12 h-12 rounded-full flex items-center justify-center text-yellow-600">
                            <ShieldCheck size={24} />
                        </div>
                        <div>
                            <h4 className="font-bold text-lg text-slate-800">Verificar Duplicados Lógicos</h4>
                            <p className="text-sm text-slate-500">Busca niños que tengan el mismo Nombre + Edad + Género (mismo niño registrado 2 veces).</p>
                        </div>
                    </div>
                    <button
                        onClick={async () => {
                            ...
                            dump.registrations.forEach((r: any) => {
                                (r.children || []).forEach((c: any) => {
                                    allKids.push({
                                        name: c.fullName?.trim().toLowerCase(),
                                        age: c.age,
                                        gender: c.gender,
                                        parent: r.parentName || r.fullName,
                                        phone: r.whatsapp,
                                        invite: c.inviteNumber
                                    });
                                });
                            });
                            ...
                            allKids.forEach(kid => {
                                const key = `${kid.name}|${kid.age}|${kid.gender}`;
                                if (seen.has(key)) {
                                    const prev = seen.get(key);
                                    duplicates.push(`${kid.name} (${kid.age} años) - Registrado por: ${prev.parent} y ${kid.parent}`);
                                } else {
                                    seen.set(key, kid);
                                }
                            });
                            ...
                            if (duplicates.length > 0) {
                                alert(`⚠️ Se encontraron ${duplicates.length} posibles duplicados:\n\n${duplicates.slice(0, 10).join('\n')}\n${duplicates.length > 10 ? '... y más' : ''}`);
                                console.table(duplicates);
                            } else {
                                alert("✅ No se encontraron duplicados lógicos (Nombre + Edad + Género).");
                            }
                        }}
                        className="px-6 py-2 bg-yellow-500 hover:bg-yellow-600 text-white font-bold rounded-lg transition-colors flex items-center gap-2"
                    >
                        <Search size={18} /> Escanear Duplicados (Nombres)
                    </button>
                </div>
            </div>
                    ...
                    viewingQR && (
                        <div className="fixed inset-0 z-[70] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
                            <div className="bg-white p-8 rounded-2xl shadow-2xl max-w-sm w-full relative flex flex-col items-center">
                                <button onClick={() => setViewingQR(null)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600">
                                    <X size={24} />
                                </button>
                                ...
                                <div className="bg-white p-4 rounded-xl shadow-inner border border-slate-100">
                                    <QRCodeCanvas value={viewingQR.data} size={200} level={"H"} includeMargin={true} />
                                </div>
                                <p className="text-xs text-slate-400 mt-6 text-center">Muestra este código al equipo de entrega para recibir el juguete.</p>
                            </div>
                        </div>
                    )
                }
