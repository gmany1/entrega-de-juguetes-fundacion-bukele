...
import {
    LayoutDashboard, FileText, Package, Landmark, Menu, X, Plus,
    Users, Calendar, ScanLine, LogOut
} from 'lucide-react';
...
const AdminLayout: React.FC<AdminLayoutProps> = ({ user, onLogout }) => {
    ...
    // Mapping of internal IDs to Paths
    // We assume /admin is the base
    ...
    const MENU_ITEMS = [
        ...
        { id: 'vendors', label: 'Proveedores', icon: <Package size={20} />, allowed: ['admin', 'planner'] },
        { id: 'audit', label: 'Reporte Final', icon: <FileText size={20} />, allowed: ['admin', 'planner'] },
        // { id: 'gallery', label: 'Galería', icon: <Image size={20} />, allowed: ['admin', 'planner'] }, // Added later or now? Let's hide for now or add to 'Reporte' section concept
    ];
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans text-slate-800">
                ...
                <div className="p-6 pt-8 lg:pt-6">
                    ...
                    <div className="space-y-2">
                        {allowedItems.map(item => {
                            ...
                            // Simple active check: exact match for root, or startsWith for sub-routes
            ...
            {/* FAB (Visible on Dashboard mainly) */}
            {
                location.pathname === '/admin' && (
                    <button
